...
class User_6 extends StatefulWidget {
  const User_6({super.key});

  @override
  State<User_6> createState() => _User_6State();
}

class _User_6State extends State<User_6> {
  List<dynamic> _arr=[];
  ...
  Future<dynamic> Help3() async{
    dynamic ans="";
    final ref = FirebaseDatabase.instance.ref("06:00pm").child("flag");
  ...
  Future<List<dynamic>> Help2() async{
    List<dynamic> arr=[];
    final ref = FirebaseDatabase.instance.ref("06:00pm");
  ...
  Future<List<dynamic>> Help() async{
    ...
    final ref = FirebaseDatabase.instance.ref("06:00pm").child("list");
    final snapshot = await ref.get();
    if (snapshot.exists) {
      snapshot.children.forEach((child) {
        print(child.value);
